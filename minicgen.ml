...
let rec gen_nonempty_params base ts =
  match ts with
  | t :: ts ->
     Backend.Datatypes.Coq_pair (positive_of_int (base+1), gen_ctype t.aTypeCtype) :: gen_nonempty_params (base+1) ts
  |  _ -> []

let gen_params base params =
  match params with
  | [t] when t.aTypeDesc = ATbuiltin Tunit -> []
  | params -> gen_nonempty_params base params
...
let gen_method_stub underlay objname m =
  ...
  let dest = builtinBase_local_ident_start in  
  let ret_type = gen_ctype mt.aMethodReturnType.aTypeCtype in
  let params =  gen_params builtinBase_local_ident_start mt.aMethodArgumentTypes in 
  { fn_return = ret_type ;
...
(* When there are multiple objects or multiple layers, we just concatenate together the code for all of them. The constructor should be None here, because it needs to be built seperately to do initialization for all layers at once. *)    
...
(* Todo: for ethereum, we need to translate compile time constants
   into assignment statements, and build a constructor method for the contract. *)	    
(*	    
...
let minicgen filename ast =
  match last_and_rest ast.aFileDeclations with
